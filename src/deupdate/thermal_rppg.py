...
class MultiFrameSuperRes:
    ...
    def push(self, frame_f32: np.ndarray):
        ...
        if len(self.buffer) > self.max_frames: self.buffer.pop(0)
    def build(self) -> Optional[np.ndarray]:
        if len(self.buffer) < max(6, self.max_frames//2): return None
        ref = self.buffer[len(self.buffer)//2]
        ref_u8 = normalize_to_uint8(ref)
        H, W = ref.shape; S = self.scale
        import cv2, numpy as np
        acc = np.zeros((H*S, W*S), dtype=np.float32); cnt = np.zeros((H*S, W*S), dtype=np.float32)
        for frm in self.buffer:
            mov_u8 = normalize_to_uint8(frm)
            try:
                ref_f = ref_u8.astype(np.float32) / 255.0; mov_f = mov_u8.astype(np.float32) / 255.0
            ...
            M_hr = M.copy(); M_hr[0,2]*=S; M_hr[1,2]*=S
...
class ThermalFaceDetector:
    def __init__(self, ambient_delta: float = 1.2, p_hot: float = 80.0,
        ...
        self.ambient_delta = ambient_delta; self.p_hot = p_hot
        self.min_area_frac = min_area_frac; self.max_area_frac = max_area_frac
        self.top_bias = top_bias; self.search_expand = search_expand
        self.lost_tolerance = lost_tolerance
        self.last_bbox = None; self.last_template = None; self.missed = 0
    def _score_components(self, frame, stats, cents, cx_img, cy_img):
        best_idx, best_score = -1, -1e9
        H, W = frame.shape
        for i in range(1, stats.shape[0]):
            x, y, bw, bh, area = stats[i]
            if area < (H*W)*self.min_area_frac or area > (H*W)*self.max_area_frac: continue
            ar = bw / max(1, bh)
            if ar < 0.5 or ar > 2.2: continue
            cx, cy = cents[i]; region = frame[y:y+bh, x:x+bw]
        ...
        x, y, bw, bh, _ = stats[best_idx]; pad = 0.12
        x = max(0, int(x - bw * pad)); y = max(0, int(y - bh * pad))
        bw = int(min(W - x, int(bw * (1 + 2*pad)))); bh = int(min(H - y, int(bh * (1 + 2*pad))))
        return (x, y, bw, bh)
    def _threshold(self, frame):
        ambient = float(np.median(frame)); t1 = ambient + self.ambient_delta; t2 = float(np.percentile(frame, self.p_hot))
        thr = max(t1, t2); return (frame >= thr).astype(np.uint8)
    def _template_track(self, frame_u8, search_bbox):
        x, y, w, h = search_bbox; search = frame_u8[y:y+h, x:x+w]
        if self.last_template is None or search.size == 0: return None
        try:
            res = cv2.matchTemplate(search, self.last_template, cv2.TM_CCOEFF_NORMED)
            _, maxv, _, maxloc = cv2.minMaxLoc(res)
            if maxv < 0.35: return None
            tx, ty = maxloc; th, tw = self.last_template.shape
            return (x+tx, y+ty, tw, th)
        except Exception: return None
    def detect(self, frame: np.ndarray):
        H, W = frame.shape; cx_img, cy_img = W*0.5, H*self.top_bias
        frame_u8 = normalize_to_uint8(frame)
        if self.last_bbox is not None and self.missed < self.lost_tolerance:
            lx, ly, lw, lh = self.last_bbox; ex = int(lw*self.search_expand); ey = int(lh*self.search_expand)
            sx = max(0, lx-ex); sy = max(0, ly-ey); sw = min(W-sx, lw+2*ex); sh = min(H-sy, lh+2*ey)
            local = frame[sy:sy+sh, sx:sx+sw]; hot = self._threshold(local)
            k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
            hot = cv2.morphologyEx(hot, cv2.MORPH_OPEN, k, iterations=1); hot = cv2.morphologyEx(hot, cv2.MORPH_CLOSE, k, iterations=2)
            ...
            if num > 1:
                ...
                if bbox is not None:
                    x,y,w,h = bbox; self.last_bbox = (sx+x, sy+y, w, h); self.missed = 0
                    roi_u8 = frame_u8[sy+y:sy+y+h, sx+x:sx+x+w]
                    if roi_u8.size >= 9: self.last_template = cv2.resize(roi_u8, (max(8, w), max(8, h)))
                    return self.last_bbox
            track = self._template_track(frame_u8, (sx, sy, sw, sh))
            if track is not None: self.last_bbox = track; self.missed = 0; return self.last_bbox
            self.missed += 1
        hot = self._threshold(frame)
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        hot = cv2.morphologyEx(hot, cv2.MORPH_OPEN, k, iterations=1); hot = cv2.morphologyEx(hot, cv2.MORPH_CLOSE, k, iterations=2)
        num, labels, stats, cents = cv2.connectedComponentsWithStats(hot, 8)
        if num <= 1: self.missed += 1; return None
        bbox = self._score_components(frame, stats, cents, cx_img, cy_img)
        if bbox is None: self.missed += 1; return None
        self.last_bbox = bbox; self.missed = 0
        x,y,w,h = bbox; roi_u8 = frame_u8[y:y+h, x:x+w]
        if roi_u8.size >= 9: self.last_template = cv2.resize(roi_u8, (max(8, w), max(8, h)))
        return self.last_bbox

class MotionCompensator:
    def __init__(self): self.ref = None; self.M = np.eye(2, 3, dtype=np.float32)
    def compensate(self, frame: np.ndarray, bbox):
        if bbox is None: return frame
        x, y, w, h = bbox; roi = frame[y:y+h, x:x+w]
        if self.ref is None: self.ref = roi.copy(); return frame
        try:
            ref_u8 = normalize_to_uint8(self.ref); roi_u8 = normalize_to_uint8(roi)
            ref_f = ref_u8.astype(np.float32) / 255.0; roi_f = roi_u8.astype(np.float32) / 255.0
            M = np.eye(2, 3, dtype=np.float32)
            criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 30, 1e-4)
            _, M = cv2.findTransformECC(ref_f, roi_f, M, cv2.MOTION_AFFINE, criteria)
            out = cv2.warpAffine(frame, M, (frame.shape[1], frame.shape[0]), flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP)
            self.M = M; self.ref = 0.9 * self.ref + 0.1 * out[y:y+h, x:x+w]; return out
        except Exception: return frame

class ROIManager:
    def __init__(self): self.buffers: Dict[str, deque] = {}
    def extract(self, frame: np.ndarray, bbox, patch=3):
        ...
        for name, (rx, ry, rw, rh) in rois.items():
            rx = max(0, min(rx, frame.shape[1]-1)); ry = max(0, min(ry, frame.shape[0]-1))
            rw = max(2, min(rw, frame.shape[1]-rx)); rh = max(2, min(rh, frame.shape[0]-ry))
            ...
            vals[name] = float(np.mean(patch_arr)); rois[name] = (rx, ry, rw, rh)
        return vals, rois

class SignalProc:
    def __init__(self, fs): self.fs = fs; self.band = (0.7, 3.0)
    def detrend_bandpass(self, x):
        x = x - np.mean(x)
        try:
            sos = signal.butter(4, self.band, btype='band', fs=self.fs, output='sos')
            x = signal.sosfilt(sos, x)
        except Exception: pass
        return x
    def hr_fft(self, x):
        if len(x) < int(8 * self.fs): return None, 0.0
        n = int(2 ** np.ceil(np.log2(len(x) * 2))); win = np.hanning(len(x))
        X = np.fft.rfft(x * win, n=n); f = np.fft.rfftfreq(n, 1/self.fs)
        m = (f >= self.band[0]) & (f <= self.band[1])
        if not np.any(m): return None, 0.0
        mag = np.abs(X[m]); fi = f[m]; pk = np.argmax(mag); hr = fi[pk] * 60.0
        conf = float(np.max(mag) / (np.mean(mag) + 1e-8)); return hr, float(np.clip(conf/10.0, 0.0, 1.0))

class RespEstimator:
    def __init__(self, fs): self.fs = fs; self.band = (0.1, 0.5)
    def detrend_bandpass(self, x):
        x = x - np.mean(x)
        try:
            sos = signal.butter(4, self.band, btype='band', fs=self.fs, output='sos')
        ...
        return x
    def rr_fft(self, x):
        if len(x) < int(12 * self.fs): return None, 0.0
        n = int(2 ** np.ceil(np.log2(len(x) * 2))); win = np.hanning(len(x))
        X = np.fft.rfft(x * win, n=n); f = np.fft.rfftfreq(n, 1/self.fs)
        m = (f >= self.band[0]) & (f <= self.band[1])
        if not np.any(m): return None, 0.0
        mag = np.abs(X[m]); fi = f[m]; pk = np.argmax(mag); rr = fi[pk] * 60.0
        conf = float(np.max(mag) / (np.mean(mag) + 1e-8)); return rr, float(np.clip(conf/10.0, 0.0, 1.0))

class MonitorUI:
    def __init__(self, up_scale: int, fs: float):
        self.up_scale = up_scale; self.fs = fs
        self.hr_hist = deque(maxlen=240); self.qual_hist = deque(maxlen=240)
        self.last_tick = time.time(); self.fps = 0.0
        self.window_name = 'Thermal rPPG Monitor'
        self.compare_mode = 'SR'; self.status_text = ''
        try: cv2.namedWindow(self.window_name, cv2.WINDOW_NORMAL)
        except Exception: logger.warning("Cannot create window (headless?). UI disabled.")
    ...
    def update(self, thermal_u8_lr: np.ndarray, bbox, roi_boxes, hr: Optional[float], q: float,
               ...
               color_rec = None):
        now = time.time(); dt = now - self.last_tick
        ...
        def overlay(vis):
            if bbox is not None:
                x,y,w,h = bbox; cv2.rectangle(vis, (x,y), (x+w, y+h), (0,255,255), 2)
            ...
            return vis
        vis_lr = overlay(vis_lr); vis_sr = overlay(vis_sr)
        base = np.hstack([vis_lr, vis_sr]) if self.compare_mode == 'SPLIT' else (vis_lr if self.compare_mode == 'LR' else vis_sr)
        h, w = base.shape[:2]; panel_w = 260
        panel = np.zeros((h, panel_w, 3), dtype=np.uint8); panel[:] = (25,25,25)

        if hr is not None: self.hr_hist.append(hr)
        ...
        def put(k, v):
            nonlocal line
            # ✅ 남은 공간 체크(한 항목에 대략 42px 사용)
            if line + 42 > h:
                return False
            cv2.putText(panel, f"{k}", (10, line), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (180,180,180), 1); line += 18
            cv2.putText(panel, f"{v}", (20, line), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (240,240,240), 1); line += 24
            return True
        ...
        if color_rec is not None:
            if line + 18 < h:
                cv2.putText(panel, f"Therapy: {color_rec.mode}", (10, line), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (180,180,180), 1)
                line += 18
            if line + 40 <= h:
                swatch = np.zeros((40, 40, 3), dtype=np.uint8)
                r,g,b = color_rec.rgb_primary; swatch[:] = (b,g,r)
        ...
        try:
            cv2.imshow(self.window_name, out); key = cv2.waitKey(1) & 0xFF
            if key == ord('q'): return 'quit'
            elif key == ord('s'): ts = time.strftime('%Y%m%d_%H%M%S'); cv2.imwrite(f'thermal_snap_{ts}.png', out)
            elif key == ord('t'): self.compare_mode = {'SR':'LR','LR':'SPLIT','SPLIT':'SR'}[self.compare_mode]
        except Exception: pass
        return None
...
class ThermalrPPG:
    def __init__(self, cfg: ThermalrPPGConfig, mqtt_pub: Optional[MqttColorPublisher] = None):
        ...
        self.up_scale = 6
        self.mfsr = MultiFrameSuperRes(scale=cfg.superres_scale, max_frames=cfg.superres_frames, deconvolution=cfg.superres_deconvolution) if cfg.enable_superres else None
    ...
    def _append(self, roi_vals: Dict[str, float]):
        for k, v in roi_vals.items():
            if k not in self.buffers: self.buffers[k] = deque(maxlen=2048)
    ...
    def _compute_hr(self):
        ...
        for name, buf in self.buffers.items():
            ...
            if hr is not None: hrs.append(hr); qs.append(conf)
        if not hrs: return None, 0.0
        w = np.array(qs); w = w / (w.sum() + 1e-6)
        return float(np.sum(np.array(hrs) * w)), float(np.mean(qs))

    def _compute_rr(self):
        if 'nose' not in self.buffers or len(self.buffers['nose']) < int(12*self.cfg.sampling_rate): return None, 0.0
    ...
    def _compute_dT(self, win_sec: int = 5):
        fs = self.cfg.sampling_rate; n = int(fs * win_sec)
        def mean_last(name):
            if name not in self.buffers or len(self.buffers[name]) < max(4, n): return None
            arr = np.array(self.buffers[name], dtype=np.float32)[-n:]
            return float(np.mean(arr))
        fh = mean_last('forehead'); nose = mean_last('nose'); lch = mean_last('l_cheek'); rch = mean_last('r_cheek')
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                H, W = frame.shape
                sr = None
                if self.mfsr is not None: self.mfsr.push(frame); sr = self.mfsr.build()
                ...
                bbox = self.detector.detect(up); tracking_ok = bbox is not None and self.detector.missed == 0
                if self.cfg.enable_motion_compensation and bbox is not None:
                    up = self.motion.compensate(up, bbox); up_lr = self.motion.compensate(up_lr, bbox)

                roi_vals, roi_boxes = self.roi.extract(up, bbox)
                if roi_vals: self._append(roi_vals)

                hr, q = (None, 0.0)
                if self._enough(): hr, q = self._compute_hr()
                rr, rrq = self._compute_rr()
                dT_nose, dT_cheek, fh_temp = self._compute_dT(win_sec=5)

                color_rec = None
                if hr is not None:
                    color_rec = self.therapist.recommend(ColorMetrics(hr=hr, q=q, rr=rr, dT_nose=dT_nose, dT_cheek=dT_cheek, forehead_temp=fh_temp))
                    ...
                    print(f"\r🫀 {hr:.1f} BPM | q={q:.2f} | 🌬 {rr_txt} brpm | ΔT_nose {dtn_txt} ΔT_cheek {dtc_txt} | 🎨 {color_rec.mode} RGB{color_rec.rgb_primary} I={color_rec.intensity:.2f} {color_rec.duration_seconds}s  ", end='', flush=True)
                    if self.mqtt is not None:
                        self.mqtt.publish_color(color_rec, metrics={"hr":hr, "rr":rr, "q":q, "dT_nose":dT_nose, "dT_cheek":dT_cheek, "forehead":fh_temp}, also_pico=True)

                if self.monitor is not None:
                    thermal_u8_sr = normalize_to_uint8(up); thermal_u8_lr = normalize_to_uint8(up_lr)
                    samples = max((len(b) for b in self.buffers.values()), default=0)
                    action = self.monitor.update(thermal_u8_lr, bbox, roi_boxes, hr, q, rr, dT_nose, dT_cheek, samples, thermal_u8_sr, tracking_ok, color_rec=color_rec)
...
if __name__ == "__main__":
    cfg = ThermalrPPGConfig(
        sampling_rate=16.0, min_measurement_duration=12, enable_motion_compensation=True,
        debug_visual=True, enable_superres=True, superres_scale=2, superres_frames=12, superres_deconvolution=True
    )
    mqtt_pub = None
    try:
