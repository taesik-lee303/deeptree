...
from connectors.mqtt.mqtt_config import settings

def on_message(client, userdata, msg):
    if msg.topic == settings.topic_status:
        print(f"[STATUS] {msg.payload.decode('utf-8', 'ignore')}")
        return
    if msg.topic == settings.topic_total:
        try:
            data = json.loads(msg.payload.decode('utf-8', 'ignore'))
        except Exception:
            print("[TOTAL] (malformed payload)")
            return
        rgb = data.get('rgb', [])
        metrics = data.get('metrics', {})
        print(f"[TOTAL] rgb={rgb} I={data.get('intensity')} mode={data.get('mode')} conf={data.get('confidence')} duration={data.get('duration')}s")
        if metrics:
            print(f"  Metrics: HR={metrics.get('hr')} RR={metrics.get('rr')} Q={metrics.get('q')} Temp={metrics.get('forehead')}")
...
def main():
    ...
    client.subscribe(settings.topic_total, qos=1)

    print(f"Subscribed to: {settings.topic_status}, {settings.topic_total}")
    try:
        client.loop_forever()
